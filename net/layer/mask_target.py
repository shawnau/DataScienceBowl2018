...
def make_one_mask_target(cfg, mode, input, proposal, truth_box, truth_label, truth_instance):
    ...
    1. assign truth box to each proposal by threshold for fg/bg
    2. crop assigned instance into bbox size
    3. resize to maskhead's output size.
    :param input: image as (H, W, C) numpy array
    :param proposal: list of regional proposals generated by RCNN. e.g.
    ...
    :param truth_label: 1s
        ...
        here we have 2 classes so it's fixed to 1
    :param truth_instance: list of truth instances, (H, W)
    :return:
        sampled_proposal: same as proposal
        sampled_label: same as truth_label
        sampled_instance: cropped instance, matching maskhead's output
        sampled_assign: index of truth_box each proposal belongs to
    """
    ...
    # filter invalid proposal like small proposals
